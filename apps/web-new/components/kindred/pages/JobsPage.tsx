...
export function JobsPage() {
  ...
  const list = useMemo(() => {
    ...
    if (active === "Remote") {
      return searchFiltered.filter((job) => job.location.toLowerCase().includes("remote"));
    }

    return searchFiltered.filter((job) => job.type === active);
          ...
          <div>
            ...
            {searchQuery ? (
              <p className="mt-2 text-sm font-semibold text-indigo-700">
                Showing results for &quot;{searchQueryValue.trim()}&quot;
              </p>
            ) : null}
          </div>
