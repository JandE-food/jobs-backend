...
export default async function CreatorProfileRoute({
  ...
  const creator = people.find((person) => person.id === id);
  ...
  const creatorPosts = feed.filter(
    (item): item is Extract<(typeof feed)[number], { kind: "post" }> =>
      item.kind === "post" && item.author.id === creator.id,
  );
    ...
    <div className="space-y-6 pb-8 pt-4">
      <Card className="overflow-hidden border-slate-200/90 bg-[radial-gradient(circle_at_top,_rgba(59,130,246,0.18),_rgba(15,23,42,0.96)_48%,_rgba(2,6,23,1))] p-6 text-white shadow-[0_28px_70px_rgba(15,23,42,0.22)]">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-4">
          <Avatar src={creator.avatar} alt={creator.name} size={88} ring />
          <div className="min-w-0">
            ...
            <div className="mt-3 flex items-center gap-2 text-sm text-white/72">
              ...
              <span>·</span>
              <span>{creator.mutuals ?? 0} mutual connections</span>
            </div>
          </div>
        </div>

        <p className="mt-6 max-w-2xl text-sm leading-7 text-white/82">
          Discover recent creator posts, focus areas, and short-form updates from {creator.name}.
        </p>
          ...
          <div className="mt-5 grid gap-4">
            {creatorPosts.length ? (
              creatorPosts.map((post) => (
                <article key={post.id} className="rounded-[1.5rem] border border-slate-200 bg-slate-50/90 p-4">
                  <div className="flex items-center justify-between gap-3">
